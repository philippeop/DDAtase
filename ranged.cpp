...
#endif
    if (&p == &u)
     animation_delay(BULLET_SPEED);//nanosleep(&ts, NULL);
     refresh_terrain(); //replaces wrefresh(w_terrain);
   }
...
#ifdef TILES
 WINDOW* w_target = newwin(13, 48, 12, 7, tiles.width*(TERRAIN_WINDOW_WIDTH + 1));
#else
...
#ifdef TILES // this will probly fuck it up
